...
if (!rex::isBackend()) {
    rex_extension::register('RESPONSE_SHUTDOWN', function(rex_extension_point $ep)
    {
        ...
        if ($timediff > 3 && filesize($logFile) > 0 && $file = new rex_log_file($logFile)) {
            ...
            $mailBody = '';
            $mailBody .= '<style> .eventbg {background: #F6C4AF; } td, th {padding: 5px;} table {width: 100%; border: 1px solid #ccc; } th {background: #b00; color: #fff;} td { border: 0; border-bottom: 1px solid #b00;} </style> ';
            $mailBody .= '<table>';
            $mailBody .= '    <thead>';
            $mailBody .= '        <tr>';
